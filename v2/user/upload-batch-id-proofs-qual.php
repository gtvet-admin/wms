...
if(isset($_POST) && !empty($_FILES['image']['name'])){
   ...
   $filepath = 'scripts/uploads/candidatePassCert/'; 
   ...
   { 
      ...
      if(move_uploaded_file($tmp,$filepath)){
		 ...
		 $sql="UPDATE `btch_2` SET pass_cert_url ='$filepath' WHERE mprId = '$mprId'";
		  //var_dump($_POST);
		  //exit;
        ...
        <div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
            <div class="card">
                <div class="card-header">
                    <center>
                        <h4>Upload Photo  For Passing Certificate</h4>
                    </center>
                </div>
                <div class="card-body">
...
$result ="SELECT *,(CASE WHEN pass_cert_url = 'pdf.png' THEN 'Photo Not Uploaded' ELSE 'Photo Uploaded' END) as phto FROM btch_2 WHERE mprId ='$prabn'";
$result=mysqli_query($con,$result);
    if ($result->num_rows > 0) {
    echo "<div style='overflow:auto;'><table class='table table-bordered mb-0'><thead><tr class='bg-primary'><th>Sl No.</th><th>Candidate ID</th><th>Candidate Name</th><th>Father Name</th><th>Photo</th><th>Upload Photo</th></tr></thead>";
      while($row = mysqli_fetch_array($result))  { 
		  ...
		  switch ($row['phto']) {
			...
			default:
				$phto_btn= "<td style='background-color:green;'><a style='color: #fff;'>Photo Already Uploaded </a></td>";
		  } 
          
        echo "<tr><td>".$row['id']."</td><td>".$row['mprId']."</td><td>".$row["fname"]."&nbsp;".$row["mname"]."&nbsp;".$row["lname"]."</td><td>".$row["fathersName"]."</td><td>"."<img src='".$row['pass_cert_url']."' width='100' height='120'/>"."</td>$phto_btn</tr>";
    }
    echo "</div></table>";
} else {
