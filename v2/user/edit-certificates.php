...
$ubn = $_REQUEST['cen_id'];
$query = "SELECT * FROM certs WHERE cen_id= '$ubn'";
...
<!-- Display status message -->
   ...
   <section class="section">
	...
	<div class="card-header">
                    <center>
                        <h4>Edit <?=$sch;?> Candidate Certificates</h4>
                    </center>
                </div>
		</br>
			</br>
        ...
        <tbody>
         <?php
            $sqlSelect = "SELECT * FROM certs WHERE cen_id = '$ubn' AND lern_prog ='$sch' AND lernern_id != '' AND industry='$ind'";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_array($result)) {
					...
					switch ($sts) {
				case "0":
					$att=  "<center><a href='scripts/cert-vrfy.php?id=$id' onclick='return Confirm()' class='btn-sm btn-block btn-success btn-upload'><i class='fa fa-unlock'></i> Confirm</a></center>";
				break;
				
				case "1":
					$att=  "<center><a href='#' class='btn-sm btn-block btn-warning btn-upload'><i class='fa fa-lock'></i> Locked</a></center>";
				break;
				
				default:
					$att= "<center><a href='#' class='btn-sm btn-block btn-danger btn-upload'><i class='fa fa-lock'></i> Confirm Status First!</a></center>";
					}
					$edit="";
					switch ($sts) {
				case "0":
					$edit=  "<center><a class='btn-sm btn-block btn-warning btn-upload' href='scripts/certs/download-$sch-certificate.php?id=$id'>Edit</center>";
				break;
				
				case "1":
					$edit=  "<center><a href='#' class='btn-sm btn-block btn-warning btn-upload'><i class='fa fa-lock'></i> Locked</a></center>";
				break;
				
				default:
					$edit= "<center><a href='#' class='btn-sm btn-block btn-danger btn-upload'><i class='fa fa-lock'></i> Confirm Status First!</a></center>";
					}
					$del="";
					switch ($sts) {
				case "0":
					$del=  "<center><a class='btn-sm btn-block btn-danger btn-upload' onclick='return Delete()' href='#'>Delete</center>";
				break;
				
				case "1":
					$del=  "<center><a href='#' class='btn-sm btn-block btn-warning btn-upload'><i class='fa fa-lock'></i> Locked</a></center>";
				break;
				
				default:
					$del= "<center><a href='#' class='btn-sm btn-block btn-danger btn-upload'><i class='fa fa-lock'></i> Confirm Status First!</a></center>";
					}
                    ?>
            <tr>
				<td><?php echo $i;?></td>
				<td><?php echo $row['lernern_id']; ?></td>
                <td><?php echo $row['fname']; ?> <?php echo $row['mname']; ?> <?php echo $row['lname']; ?></td>
				<td><?php echo $row['mob_no']; ?></td>
				<td><?php echo $row['lern_course']; ?></td>
				<td><?php echo $att;?></td>		
				<td><?php echo $edit;?></td>
				<td><?php echo $del;?></td>
...
function Delete(){
    return confirm('Are you sure want to Candidate of <?=$mpr?>, Once Confirmed You Wont be able to get it back!');
}
</script><script language="JavaScript" type="text/javascript">
function Confirm(){
    return confirm('Are you sure want to Confirm Candidate of <?=$mpr?>, Once Confirmed You Wont be able to make any changes!');
}
</script>
<!--Jquery.min js-->
