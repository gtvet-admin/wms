<?php
    ...
    if (isset($_GET['ro_email'])) {

        $ro_email = $_GET['ro_email'];//Set the data from GET

    }

    //Get Data from the table to verify the user
    $getUserDataQuery = "SELECT * FROM `onboarding` WHERE `ro_email` LIKE '$ro_email'";
    ...
    if($getUserDataResult->num_rows > 0) { //User Exists
        ...
        $org_name= $getUserData['org_name']; //Get the verificationID
        ...
        if($verificationIDFromMail == $verificationID) { //Verification ID Matched
            ...
            $updateUserDataQuery = "UPDATE `onboarding` SET `password` = '$generatedPassword', `emailVerified`='Yes',`verificationID`='',`lastUpdatedOn`='$lastUpdatedOn' WHERE `ro_email` LIKE '$email'";
            ...
            if ($updateUserDataResult) { //if update successful
                ...
                $mail->AddAddress($ro_email);
                $mail->AddCC("anuj@gramtarang.org.in");
                //$mail->AddBCC("vaibhav.bhouraskar@gramtarang.org.in");
                //$mail->AddBCC("abhinav.madan@gramtarang.org.in");
                ...
                $emailBody = str_replace("InsertNameHere", "$org_name", $emailBody); //Now Replace the string with Name
                $emailBody = str_replace("InsertUserNameHere", "$ro_email", $emailBody); //Now Replace the string with Username
        ...
        header('Location: ../../reg.php?message= Please Register Yourself before verifying');
