        ...
        <div class="dropdown">
            <a class="nav-link pl-2 pr-2 leading-none d-flex" data-toggle="dropdown" href="#">
                ...
                <span class="ml-2 d-lg-block">
                    ...
                    <span class="text-muted app-sidebar__user-name text-sm">On-Site Trainer</span>
                </span>
            </a>
        </div>
    </div>
...
<!--        </li>-->
		...
		<li class="slide">
            <a class="side-menu__item" data-toggle="slide" href="#"><span class="side-menu__label">ITC Management</span><i class="angle fa fa-angle-right"></i></a>
            <ul class="slide-menu">
			
                <?php
include ('scripts/dbcon.php');
$spoc_code = $_SESSION['spoc_code'];
$result ="SELECT * FROM opt_scheme where cen_id ='$cen_id' AND mod_1 = 'Yes' GROUP BY scheme_name";
$result=mysqli_query($con,$result);
    if ($result->num_rows > 0) {
    echo "";
      while($row = mysqli_fetch_array($result))  { 
        echo "<li><a href='add-".$row["scheme_name"]."-itc.php' class='slide-item'>".$row["scheme_name"]." ITC Details</a></li>";
    }
    echo "";
} else {
    echo "";
...
include ('scripts/dbcon.php');
$spoc_code = $_SESSION['spoc_code'];
$result ="SELECT * FROM opt_scheme where cen_id ='$cen_id' AND mod_2 = 'Yes' GROUP BY scheme_name";
$result=mysqli_query($con,$result);
    if ($result->num_rows > 0) {
    echo "";
      while($row = mysqli_fetch_array($result))  { 
...
include ('scripts/dbcon.php');
$spoc_code = $_SESSION['spoc_code'];
$result ="SELECT * FROM opt_scheme where cen_id ='$cen_id' AND mod_3 = 'Yes' GROUP BY scheme_name";
$result=mysqli_query($con,$result);
    if ($result->num_rows > 0) {
    echo "";
      while($row = mysqli_fetch_array($result))  { 
...
include ('scripts/dbcon.php');
$spoc_code = $_SESSION['spoc_code'];
$result ="SELECT * FROM opt_scheme where cen_id ='$cen_id' AND mod_4 = 'Yes' GROUP BY scheme_name";
$result=mysqli_query($con,$result);
    if ($result->num_rows > 0) {
    echo "";
      while($row = mysqli_fetch_array($result))  { 
...
include ('scripts/dbcon.php');
$spoc_code = $_SESSION['spoc_code'];
$result ="SELECT * FROM opt_scheme where cen_id ='$cen_id' AND mod_5  = 'Yes' GROUP BY scheme_name";
$result=mysqli_query($con,$result);
    if ($result->num_rows > 0) {
    echo "";
      while($row = mysqli_fetch_array($result))  { 
