            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <center><h4>Your List of Candidate Details</h4></center>
                    </div>
                    <div class="card-body">
...
$cen_id = $_SESSION['cen_id'];
$sql ="SELECT *, COUNT(cen_id) as ttl_cnt, COUNT(CASE WHEN photo != 'no-image.png' THEN '1' ELSE NULL END) as ttl_btch , COUNT(CASE WHEN photo = 'no-image.png' THEN '1' ELSE NULL END) as ttl_unass FROM btch_2 where cen_id ='$cen_id'";
$result=mysqli_query($con,$sql);
    if ($result->num_rows > 0) {
    echo "<div style='overflow:auto;'><table class='table table-bordered mb-0'><thead><tr class='bg-primary'><th>Center Name</th><th>Total Candidates Uploaded</th><th>Total Photos Uploaded</th><th>Total Photos Un-uploaded</th><th>View Candidates</th></tr></thead>";
      while($row = mysqli_fetch_array($result))  { 
 
        echo "<tr><td>".$org_name."</td><td>".$row["ttl_cnt"]."</td><td>".$row["ttl_btch"]."</td><td>".$row["ttl_unass"]."</td><td><a href='upload-batch-id-proofs.php?cen_id=".$row["cen_id"]."'>View</a></td></tr>";
    }
    echo "</div></table>";
} else {
