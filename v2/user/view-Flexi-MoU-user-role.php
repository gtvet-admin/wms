<?php
    ...
    if($role == "IP") {
        ...
        $selectUserQuery = "SELECT * FROM  cen_list a INNER JOIN onboarding b ON a.cen_code = b.cen_id WHERE a.cen_code = '$cen_id'";
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <center><h4><?=$org_name;?></h4></center>
                    </div>
                    <div class="card-body">
                        <?php
include ('scripts/dbcon.php');
$cen_id = $_REQUEST['cen_code'];
$result ="SELECT * FROM  cen_list a INNER JOIN onboarding b ON a.cen_code = b.cen_id WHERE a.cen_code = '$cen_id'";
$result=mysqli_query($con,$result);
    if ($result->num_rows > 0) {
    echo "<table class='table table-bordered mb-0'><thead><tr class='bg-primary'><th>Centre UID</th><th>Role</th><th>Contact Name </th><th>Email ID </th><th>Status</th></tr></thead>";
		  ...
		  switch ($row['role']) {
			case "TP":
			   $rl=  "Training Partner";
				...
				case "CM":
			   $rl= "On-Site Supervisor";
				break;

			default:
				$rl= "No Role Assigned! Please Contact Admin!!!";
		  } 
        echo "<tr><td>".$row["cen_code"]."</td><td>$rl</td><td>".$row["pc_name"]."</td><td>".$row["ro_email"]."</td><td>".$row["status"]."</td></tr>";
		}
    echo "</div></table>";
} else {
