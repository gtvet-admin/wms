...
<?php
include 'scripts/dbcon.php';
$row_roll=mysqli_fetch_array(mysqli_query($con,"SELECT max(id) as ref_id, cen_id as cen FROM btch_2"));
$incr_id2=$row_roll['ref_id']+1;
$id = $row_roll['id'];
$serial = str_replace("-","",$row_roll['cen']);
$mprid="".$serial.$incr_id2;
?>
<section class="section">
	...
	<div class="card-header">
                    <center>
                        <h4>View Trainee Data Uploaded</h4>
                    </center>
                </div>
		</br>
			</br>
        ...
        <tbody>
         <?php
			$ubn = $_REQUEST['cen_id'];
            $sqlSelect = "SELECT * FROM btch_2 WHERE cen_id = '$ubn' AND mprId = 'NULL'";
            ...
            <tr>
				...
				<td><center><a class='btn-sm btn-block btn-success btn-upload' href='scripts/mprid.php?mprId=<?php echo $mprid;?>&id=<?php echo $row['id'];?>'>Generate</a></center></td>
...
<section class="section">
	...
	<div class="card-header">
                    <center>
                        <h4>Recently Generated Trainee ID</h4>
                    </center>
                </div>
		</br>
			</br>
        ...
        <tbody>
         <?php
			$ubn = $_REQUEST['cen_id'];
			$dt=date("d-m-Y");
            $sqlSelect = "SELECT * FROM btch_2 WHERE cen_id = '$ubn' AND gen_dt = '$dt'";
            $result = mysqli_query($conn, $sqlSelect);
            $i=1;
            if (mysqli_num_rows($result) > 0) {
                ?>
