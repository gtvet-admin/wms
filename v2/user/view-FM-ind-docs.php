        ...
        <div class="col-lg-12 col-xl-12 col-md-12 col-sm-12">
            <div class="card">
                <div class="card-header">
                    <center>
                        <h4>View Industry Partner Documents</h4>
                    </center>
                </div>
                <div class="card-body">
...
$result ="SELECT *,(CASE WHEN agr_url = 'NULL' THEN 'Doc Not Uploaded' ELSE 'Doc Uploaded' END) as phto,(CASE WHEN wc_con = 'NULL' THEN 'Doc Not Uploaded' ELSE 'Doc Uploaded' END) as wc FROM ip_details WHERE cen_id ='$prabn'";
$result=mysqli_query($con,$result);
    if ($result->num_rows > 0) {
    echo "<div style='overflow:auto;'><table class='table table-bordered mb-0'><thead><tr class='bg-primary'><th>Sl No.</th><th>Document Name</th><th>Document Upload Status</th><th>Timestamp</th><th>Document Type</th><th>Actions</th></tr></thead>";
      while($row = mysqli_fetch_array($result))  { 
          $phto_btn="";
		  switch ($row['phto']) {
			case "Doc Uploaded":
			   $phto_btn=  "<td>Contract Agreement<td>Yes</td><td></td><td><img src='assets/img/pdf.png' style='width:100; height:120px;' frameborder='0'/></td><td>"."<a target='_blank' href='".$row['agr_url']."'>View</a>"."</td>";
				break;
				
				case "Doc Not Uploaded":
			...
			default:
				$phto_btn= "<td style='background-color:blue;'><a style='color: #fff;'>Contact Lead Co-ordinator </a></td>";
		  } 
		  ...
		  switch ($row['wc']) {
			case "Doc Uploaded":
			   $wc_btn=  "<td>WC Policy<td>Yes</td><td></td><td><img src='assets/img/pdf.png' style='width:100; height:120px;' frameborder='0'/></td><td>"."<a target='_blank' href='".$row['wc_con']."'>View</a>"."</td>";
				break;
				
				case "Doc Not Uploaded":
			   $wc_btn= "<td><form action='' enctype='multipart/form-data'  method='post'><input type='file' name='image' /><input name='mprId' type='hidden' value='".$row['mprId']."'></br><button class='btn-sm btn-block btn-primary btn-upload'>Upload</button></form></td>";
				break;

			default:
				$wc_btn= "<td style='background-color:blue;'><a style='color: #fff;'>Contact Lead Co-ordinator </a></td>";
		  } 
          
        echo "<tr><td>1</td>$phto_btn</tr>
			  <tr><td>2</td>$wc_btn</tr>";
    }
    echo "</div></table>";
} else {
