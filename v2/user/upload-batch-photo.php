...
if(isset($_POST) && !empty($_FILES['image']['name'])){
   ...
   $filepath = 'scripts/uploads/candidatePhotos/'; 
   ...
   { 
      ...
      if(move_uploaded_file($tmp,$filepath)){
		 ...
		 $sql="UPDATE `btch_2` SET photo ='$filepath' WHERE mprId = '$mprId'";
		  //var_dump($_POST);
		  //exit;
...
include ('scripts/dbcon.php');
$cen_id = $_SESSION['cen_id'];
$result ="SELECT *,(CASE WHEN photo != 'no-image.png' THEN 'Photo Uploaded' ELSE 'Photo Not Uploaded' END) as phto FROM btch_2 WHERE cen_id ='$cen_id' AND emp_status = 'Continuing'";
$result=mysqli_query($con,$result);
$i=1;
    if ($result->num_rows > 0) {
    echo "<div style='overflow:auto;'><table id='example' class='table table-bordered mb-0'><thead><tr class='bg-primary'><th>Sl No.</th><th>Candidate ID</th><th>Candidate Name</th><th>Father Name</th><th>Photo</th><th>Upload Photo</th></tr></thead>";
      while($row = mysqli_fetch_array($result))  { 
		  ...
		  switch ($row['phto']) {
			case "Photo Not Uploaded":
			   $phto_btn=  "<td><form action='' enctype='multipart/form-data'  method='post'><input type='file' name='image' /><input name='mprId' type='hidden' value='".$row['mprId']."'></br><button class='btn-sm btn-block btn-primary btn-upload'>Upload</button></form></td>";
				...
				case "Photo Uploaded":
			   $phto_btn= "<td style='background-color:blue;'><a style='color: #fff;'>Photo Uploaded</a></td>";
				break;
        ...
        echo "<tr><td>$i</td><td>".$row['mprId']."</td><td>".$row["fname"]."</td><td>".$row["fathersName"]."</td><td>"."<img src='".$row['photo']."' width='100' height='120'/>"."</td>$phto_btn</tr>";
    $i++;
	}
