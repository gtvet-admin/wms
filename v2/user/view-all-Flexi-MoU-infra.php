            ...
            <div class="col-12">
                ...
                <div class="card">
                    <div class="card-header">
                        <center><h4>Flexi MoU Exam Centre List</h4></center>
                    </div>
                    <div class="card-body">
                        <?php
include ('scripts/dbcon.php');
$cen_id = $_SESSION['cen_id'];
$result ="SELECT * FROM exm_cen_list where cen_id ='$cen_id'";
$result=mysqli_query($con,$result);
    //var_dump($cen_id);
	//exit;
	if ($result->num_rows > 0) {
    echo "<table class='table table-bordered mb-0'><thead><tr class='bg-primary'><th>Exam Centre UID</th><th>Exam Centre Name</th><th>Address</th><th>Status</th><th>Action</th><th>Assign batches</th></tr></thead>";
      while($row = mysqli_fetch_array($result))  { 
        echo "<tr><td>".$row["cen_code"]."</td><td>".$row["cen_name"]."</td><td>".$row["cen_add"]."</td><td>".$row["status"]."</td><td><a href='view-exam-infra.php?center_id=".$row["cen_id"]."'>View</a></td><td><a href='add-exam-infra.php?center_id=".$row["cen_id"]."'>Assign</a></td></tr>";
		}
    echo "</div></table>";
} else {
    echo "<center><h4>No Exam Centre Details Added! Please add Exam Centre First</h4></center>";
}
mysqli_close($con);
?>
                    </div>
